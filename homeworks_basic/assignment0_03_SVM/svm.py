...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp(-0.5*torch.pow(torch.cdist(x_1, x_2, p=2),2))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return torch.Tensor(x_1 @ torch.t(x_2)).type(torch.float32) ### YOUR CODE HERE
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                loss = self.lmbd * torch.t(self.betas[batch_inds]) @ k_batch @ self.betas + hinge_loss(preds, y_batch)
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return K@self.betas + self.bias ### YOUR CODE HERE # compute the margin values for every object in the batch
